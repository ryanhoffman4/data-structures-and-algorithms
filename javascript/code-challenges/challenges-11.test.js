...
function transformToLis(obj){
  ...
  {
    list_array.push(`<li>${key}: ${value}</li>`);
  }
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let array of input) {
    let filtered_array = [];
    filtered_array = array.filter(x => (x % 5 === 0 && typeof(x) != 'string'));
    let mapped_array = [];
    mapped_array = filtered_array.map(x => Math.pow(2, x));
    last_array.push(mapped_array);
  }
  return last_array;
};
...
let findShortest = (data) => {
  ...
  let height_array = data.map(x => x.height);

  let minHeight = height_array.reduce((minimum, currentValue) => {
